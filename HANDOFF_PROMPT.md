...
Preflight Runner concurrency at six and require all six initial Jobs to reach overlapping active
states; do not call queued work active.
...
architecture artifact and two hosted design regions, review all three drafts with both design
notes grouped under Claude, add the scripted overall instruction, and press Send once. Then show
the addressed architecture revision, any real pull-request evidence inside Guild Evidence Focus,
immutable liquid-glass V2, V1/V2 comparison, authenticated approval in the new workspace, and the
selection-scoped cited Cinemaverse answer without navigating away from Guild or opening GitHub.

Use private Cinemaverse repository baseline `a0d2339` as honest source evidence. The reserved
`cinemaverse-v1.vercel.app` and `cinemaverse-v2.vercel.app` aliases are public and previously passed
all six direct-route checks; recheck 200 responses, Guild embedding, and fresh captures before use.
Never place a protection-bypass token in Guild metadata. The
optional Preview Bridge may be absent: use revision/route/viewport/coordinate anchors, make no
stable DOM-element claim, and do not show an unavailable warning. Disconnect Chrome automation
before capture so its debugging banner is absent.
...
Perform only deterministic, non-model preflight from the clean baseline. Do not run Claude/Codex,
dispatch feedback, or publish the new workspace's design revisions before capture. The first real
Team Run and revision flow must be recorded in one normal-speed silent master using the
demo-video-producer skill and `DEMO_VIDEO_SCRIPT.md`. Inspect representative frames and verify the
media technically. If presenter face/audio is not yet available, finish everything else and report
the exact files still needed.
